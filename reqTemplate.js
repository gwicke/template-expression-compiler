...
var url = require('url');
var templateExpressions = require('./ParserWrapper');
...
function splitAndPrepareTAsseblyTemplate(templateSpec, defaultLookupPath) {
    var result = [];
    var templateNest = 0;
    var startIndex = 0;
    var currentTemplate;
    for (var index = 0; index < templateSpec.length; index++) {
        if (templateSpec[index] === '{') {
            ...
            if (templateNest === 1) { // The current template is finished
                ...
                result.push(['raw', templateExpressions.parse(currentTemplate, {
                    ctxMap: {
                        $local: 'rm.request.' + defaultLookupPath
                    }
                })]);
                startIndex = index + 1;
...
function compileTemplate(templateSpec, setValue, reqPart) {
    var res;
    var template = splitAndPrepareTAsseblyTemplate(templateSpec, reqPart);
